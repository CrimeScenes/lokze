...
local function UpdateFOV()
    ...
    Circle.Visible = getgenv().CamLock.Radius_Visibility  -- Check if the radius visibility is enabled
    Circle.Radius = getgenv().CamLock.FOV  -- Set the radius to the FOV value
    Circle.Position = Vector2.new(Mouse.X, Mouse.Y + game:GetService("GuiService"):GetGuiInset().Y)  -- Update position based on mouse
end
